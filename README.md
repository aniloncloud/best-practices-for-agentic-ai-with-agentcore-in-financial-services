...
| 0:08–0:21 | 13 min | **Lab 1** — Deploy to the AgentCore Harness: deploy, read during wait, invoke, right-size model live, session-isolation A/B, one CloudWatch trace |
| 0:21–0:39 | 18 min | **Lab 2** — Connect Tools with Gateway + JWT Auth: env block from SSM, gateway with JWT authorizer, Lambda targets, attach gateway to harness, harness authorizer config, no code edit (Identity threads identity), deploy, token, bearer invoke, 401 proof, 5 000-share trade succeeds |
...
|--------|-----|---------------------|
| 1 | Lab 1 — Harness | CISO five questions + harness.json tour |
...
- **Zero code edits.** The agent is a declarative harness (`app/PortfolioAdvisor/harness.json`). Every change in the session is configuration or a CLI command; AgentCore Identity threads the caller's identity to Gateway tools, so no auth-forwarding code is needed.
