...
| **Identity provider (IdP)** | Amazon Cognito user pool with M2M client, web client, and test user (`workshopuser@example.com` / `WorkshopPass1!`) |
| **Lambda functions** | `workshop-check-portfolio-risk` and `workshop-execute-trade` — ready to be wired through Gateway |
| **AgentCore harness IAM** | Harness execution role and gateway service role (Lambda invoke + policy-engine attach) — pre-created so deploys and governance "just work" (you create the M2M credential provider yourself in Lab 2) |
...
**Get to Lab 1 and kick off the deploy.** The deploy takes 2–3 minutes and the lab is written so you read while it runs. Don't spend time here.
:::
...
| [Enterprise Tool Registry](../35-lab2b-tool-registry/) | Enterprise Tool Registry | ~15 min | Publish, govern, and discover MCP servers org-wide with AWS Agent Registry (builds on Lab 2's Gateway tools) |
