...
**Provisioning note — Enterprise Tool Registry:** The devbox provisioning script (`static/devbox.yaml`) auto-creates an AWS Agent Registry (`PortfolioAdvisorToolRegistry`, auto-approval **off**) per account using the same `aws bedrock-agentcore-control` CLI the lab uses (guaranteed API support, unlike a Lambda's bundled SDK), and publishes its ID to SSM at `/app/portfolioadvisor/agentcore/tool_registry_id`. It runs in the **background**, is **idempotent** (skips if the param or a same-named registry already exists), and is **non-blocking** (logs to `/var/log/tool-registry-provision.log`; on any failure the optional lab simply skips when the param is absent — it never blocks boot). The code-server instance role is granted `bedrock-agentcore:CreateRegistry/GetRegistry/ListRegistries` + `ssm:PutParameter` for this. **No participant IAM change is needed** — registry actions use the `bedrock-agentcore:` prefix (the CLI is named `bedrock-agentcore-control`), already covered by `BedrockAgentCoreFullAccess` (`bedrock-agentcore:*`) in `iam_policy.json`.
